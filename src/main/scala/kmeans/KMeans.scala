...
class KMeans extends KMeansInterface:
  ...
  /*def classify(points: Seq[Point], means: Seq[Point]): Map[Point, Seq[Point]]=
    def recMap(res:List[(Point, Point)],ps: Seq[Point]): List[(Point, Point)] = 
      if ps.isEmpty then res
      else recMap((findClosest(ps.head,means),ps.head) :: res, ps.tail)
       
    recMap(Nil,points).groupMap(_._1)(_._2)   */

  def classify(points: ParSeq[Point], means: ParSeq[Point]): ParMap[Point, ParSeq[Point]] =
    if points.isEmpty then means.map(mean => (mean -> ParSeq[Point]())).toMap
  ...
  /*def update(classified: Map[Point, Seq[Point]], oldMeans: Seq[Point]): Seq[Point] =
    classified.map((oldMean, seqPoints) => findAverage(oldMean, seqPoints)).toSeq*/
  ...
  /*def converged(eta: Double, oldMeans: Seq[Point], newMeans: Seq[Point])=
    var cvgd = false
    var idx = 0
    val until = oldMeans.length
    while idx < until do
      cvgd = oldMeans(idx).squareDistance(newMeans(idx)) <= eta
      idx += 1
    cvgd*/
