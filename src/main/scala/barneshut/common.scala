...
sealed abstract class Quad extends QuadInterface :
  ...
  def isInside(q: Quad, b: Body): Boolean =
...
case class Fork(
  ...
  def insert(b: Body): Fork = b match {
    case b if isInside(nw, b) => Fork(nw.insert(b), ne, sw, se)
    case b if isInside(ne, b) => Fork(nw, ne.insert(b), sw, se)
    case b if isInside(sw, b) => Fork(nw, ne, sw.insert(b), se)
    case b if isInside(se, b) => Fork(nw, ne, sw, se.insert(b))
  }

case class Leaf(centerX: Float, centerY: Float, size: Float, bodies: coll.Seq[Body])
  extends Quad :
  val m = bodies.foldLeft(0f)((mass, body) => mass + body.mass)
  val mX = bodies.foldLeft(0f)((massXTotal, body) => massXTotal + (body.mass * body.x)) / m
  val mY = bodies.foldLeft(0f)((massYTotal, body) => massYTotal + (body.mass * body.y)) / m
  val (mass, massX, massY) = (m: Float, mX: Float, mY: Float)
  val total: Int = bodies.length

  def insertBodies(fork: Fork, bodies: coll.Seq[Body]): Fork =
    if bodies.isEmpty then fork
    else insertBodies(fork.insert(bodies.head), bodies.tail)
  ...
  def insert(b: Body): Quad =
    ...
    if size > minimumSize then
      ...
      insertBodies(Fork(nw, ne, sw, se), newBodies)
...
class SectorMatrix(val boundaries: Boundaries, val sectorPrecision: Int) extends SectorMatrixInterface :
  ...
  def +=(b: Body): SectorMatrix =
    val y = (b.x / boundaries.maxX * sectorPrecision)
    val x = (b.y / boundaries.maxY * sectorPrecision)
    this.apply(y.toInt, x.toInt) += b
    this

  def apply(x: Int, y: Int) = matrix(y * sectorPrecision + x)

  def combine(that: SectorMatrix): SectorMatrix =
    for (idx <- 0 until matrix.length)
